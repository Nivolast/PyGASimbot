...
def after_simulation(simbot: Simbot):
    ...
    for robot in simbot.robots:
        ...
        robot.fitness += simbot.eat_count * 500
        robot.fitness += simbot.score * 100
    ...
    for _ in range(num_robots - 1):
        ...
        # Crossover
        child = StupidRobot()
        crossover_point = random.randint(1, StupidRobot.NUM_RULES - 1)
        child.RULES = parent1.RULES[:crossover_point] + parent2.RULES[crossover_point:]
        ...
        for i in range(StupidRobot.NUM_RULES):
            for j in range(StupidRobot.RULE_LENGTH):
                if random.random() < mutation_rate:
                    child.RULES[i][j] = random.randrange(256)
    ...
    write_rule(simbot.robots[0], "PyGASimbot 2021/PyGASimbot/Rule_log/best_gen{0}.csv".format(simbot.simulation_count))
